...
function addNewGroup(){
    ...
    $uamUserGroup->setGroupTitle($grp_title);
	$uamUserGroup->setGroupName($grp_name);
	$uamUserGroup->setGroupDesc($grp_desc);
	$uamUserGroup->setGroupHash($grp_name);
	$uamUserGroup->save();
	...
	*/
	...
	$categoryID = _setup_group_category_association($groupID, $grp_name, $grp_desc, $grp_email);
	...
	_make_user_group_owner($wpdb, $current_user->ID, $groupID);

	echo "New group succesfully created.";

	$group_hash = $uamUserGroup->getGroupHash();
	$email_ids = explode(",", $grp_invite);
	$mail = new PHPMailer(true);
	$send_email = false;
	if(count($email_ids) > 0){
			foreach($email_ids as $id){
			 if($id == $current_user->user_email){
			 	//The current user i.e. creator of the group has already been made a member of the group.
			 	continue;
			 }
			 $send_email = true;
			 $mail->AddAddress($id);
			 setMemberRequestForGroup($wpdb, $id, $groupID);
		}
	}
	if($send_email){
		try {	 
		  ...
		  $mail->AltBody = 'To view the message, please use an HTML compatible email viewer!'; // optional - MsgHTML will create an alternate automatically
		  $mail->MsgHTML('<p>Your have been added to the group '.$grp_title.' by'.$current_user->user_firstname.' '.$current_user->user_lastname.'</p><br><p>Goto http://vinpad.us/wp/confirmaddtogroup?group='.$group_hash.' to confirm that you want to join the group</p>');
		  $mail->Send();
...
function setMemberRequestForGroup($wpdb, $emailID, $groupID){

	//TODO: This has to be done in future.
	//_check_if_user_exists($uname);
	//_check_if_group_exists($grpname);
	$set_mem_request_sql = $wpdb->prepare("INSERT INTO `wp_uam_accessgroups_requests` (group_id, email_id) 
	VALUES ( %d, %s )", $groupID, $emailID);
	$wpdb->query($set_mem_request_sql);
}

function _setup_group_category_association($groupID, $grp_name, $grp_desc, $grp_email){
...
function _set_user_group_access($wpdb, $userID, $groupID){
	...
	return $wpdb->query($group_cat_sql);
}

function _make_user_group_owner($wpdb, $userID, $groupID){
	//IMPORTANT NOTE: Update database names (like wp_uam_accessgroups for example) at all places
	//in order to make the application portable, when moved to a different database or the table
	//names are changed.
	$update_group_owner_sql = $wpdb->prepare("UPDATE `wp_uam_accessgroups` SET group_owner=%d where ID=%d", $userID, $groupID);
	$wpdb->query($update_group_owner_sql);
...
function postToGroups(){
	...
	if($_group){
		foreach($_group as $category_to_publish){
			...
			if (is_user_logged_in()){
						...
						$my_post = array(
									 ...
									 'post_category' => array($category_to_publish)
								  );
...
/*
 * Takes the hash value and returns the corresponding group ID.
 */
function _get_group_id_from_hash($hash){
	//Function to get the ID of the group from hash value
	global $wpdb;
	$group_id_from_hash_sql = $wpdb->prepare("SELECT ID as group_id FROM `wp_uam_accessgroups` WHERE group_hash=%s", $hash);
	return $wpdb->get_var($group_id_from_hash_sql);
}

function _check_user_group_request($userID, $groupID){
	//Function to check if the user has a pending request to be added to the group.
	//Returns true if a match is found. False otherwise.
	global $wpdb;
	$email_group_sql = $wpdb->prepare("SELECT email_id FROM `wp_uam_accessgroups_requests` WHERE group_id=%d", $groupID);
	$email_group_id = $wpdb->get_var($email_group_sql);
	
	$email_user_sql = $wpdb->prepare("SELECT user_email FROM `wp_users` WHERE ID=%d", $userID);
	$email_user_id = $wpdb->get_var($email_user_sql);
	return ($email_group_id == $email_user_id);
	
}

function confirm_user_add_to_group(){
	global $wpdb;
	//Get the ID of the current user. Is the most
	//secure way of doing this? Can there be a situation
	//where the user who clicked on confirm is a different user
	//to the user who is actually calling this API.
	
	$groupID = _get_group_id_from_hash($_POST['group_added_to_hash']);
	//Check if the current user has been added to the group request table
	global $current_user;
    get_currentuserinfo();
    $current_user_id = $current_user->ID;
	if(_check_user_group_request($current_user_id, $groupID)){
		//User has a pending request for this group. So we go ahead and grant him ownership.
		$ret= _set_user_group_access($wpdb, $current_user_id, $groupID);
		
		if($ret){
			echo "Congrats! You are now part of the group. Use the bookmarklet to share stuff to the group";
		}
		else
		{
			echo "Error while adding to the group. :( Please try again.";
		}
	}
	else
	{
		echo "Sorry! You donot have permissions to join this group! Please request to be added to the group at <a href='http://vinpad.us/wp/requestGroupMembership'>Request Group Access</a>";
	}
...
add_action('wp_ajax_nopriv_makeMemberofGroup', 'makeMemberofGroup');
add_action('wp_ajax_postToGroups', 'postToGroups');
add_action('wp_ajax_nopriv_postToGroups', 'postToGroups');
add_action('wp_ajax_confirm_user_add_to_group', 'confirm_user_add_to_group');
add_action('wp_ajax_nopriv_confirm_user_add_to_group', 'confirm_user_add_to_group');